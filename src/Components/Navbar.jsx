...
const Navbar = () => {
  ...
  return (
    <nav className='w-full h-[70px] flex items-center ms:justify-around justify-between fixed border-b-2 px-6 z-10 z-100'>
        <div className="w-[100px] flex items-center justify-between z-10">
          ...
          <ul className="navbar hidden ms:block">
          ...
          <div className={`w-full h-[58vh] top-0 left-0 fixed bg-white border-b-2 shadow-lg ${toggle  ? 'block' : 'hidden px-4'}`}>
        ...
        {toggle ? <IoMdClose className='text-2xl cursor-pointer block ms:hidden z-10' onClick={clickToggle} /> : <HiOutlineMenuAlt4 className='text-2xl cursor-pointer block ms:hidden z-10' onClick={clickToggle} />}
    </nav>
  )
}
